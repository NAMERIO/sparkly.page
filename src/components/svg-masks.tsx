// TODO: remove unused
export function SvgMasks() {
	return (
		<div>
			<svg
				aria-hidden="true"
				style={{
					height: "1px",
					left: "-1px",
					pointerEvents: "none",
					position: "absolute",
					top: "-1px",
					width: "1px",
				}}
				viewBox="0 0 1 1"
			>
				<mask
					id="svg-mask-squircle"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<path
						d="M0 0.464C0 0.301585 0 0.220377 0.0316081 0.158343C0.0594114 0.103776 0.103776 0.0594114 0.158343 0.0316081C0.220377 0 0.301585 0 0.464 0H0.536C0.698415 0 0.779623 0 0.841657 0.0316081C0.896224 0.0594114 0.940589 0.103776 0.968392 0.158343C1 0.220377 1 0.301585 1 0.464V0.536C1 0.698415 1 0.779623 0.968392 0.841657C0.940589 0.896224 0.896224 0.940589 0.841657 0.968392C0.779623 1 0.698415 1 0.536 1H0.464C0.301585 1 0.220377 1 0.158343 0.968392C0.103776 0.940589 0.0594114 0.896224 0.0316081 0.841657C0 0.779623 0 0.698415 0 0.536V0.464Z"
						fill="white"
					/>
				</mask>
				<mask
					id="svg-mask-header-bar-badge-top"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" x="0" y="0" />
					<circle cx="0.75" cy="0.25" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-header-bar-badge-bottom"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" x="0" y="0" />
					<circle cx="0.75" cy="0.75" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-voice-user-summary-item"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<circle
						cx="1.2083333333333333"
						cy="0.5"
						fill="black"
						r="0.5416666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-vertical-fade"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<linearGradient
						gradientTransform="rotate(90)"
						id="svg-mask-vertical-fade-gradient"
						x1="0"
						x2="1"
						y1="0"
						y2="0"
					>
						<stop offset="0%" stopColor="white" />
						<stop offset="100%" stopColor="black" />
					</linearGradient>
					<rect
						fill="url(#svg-mask-vertical-fade-gradient)"
						height="1"
						width="1"
						x="0"
						y="0"
					/>
				</mask>
				<mask
					id="svg-mask-panel-button"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" x="0" y="0" />
					<circle cx="0.75" cy="0.75" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-channel-call-control-button"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle
						cx="0.8214285714285714"
						cy="0.8214285714285714"
						fill="black"
						r="0.25"
					/>
				</mask>
				<mask
					id="svg-mask-channel-call-control-button-badge-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.42857142857142855"
						rx="0.21428571428571427"
						ry="0.21428571428571427"
						width="0.42857142857142855"
						x="0.6428571428571429"
						y="-0.07142857142857142"
					/>
				</mask>
				<mask
					id="svg-mask-channel-call-control-button-badge-22"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.42857142857142855"
						rx="0.21428571428571427"
						ry="0.21428571428571427"
						width="0.5357142857142857"
						x="0.5357142857142857"
						y="-0.07142857142857142"
					/>
				</mask>
				<mask
					id="svg-mask-channel-call-control-button-badge-29"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.42857142857142855"
						rx="0.21428571428571427"
						ry="0.21428571428571427"
						width="0.6607142857142857"
						x="0.4107142857142857"
						y="-0.07142857142857142"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-default"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
				</mask>
				<mask
					id="svg-mask-avatar-status-round-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.8125" cy="0.8125" fill="black" r="0.3125" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7604166666666667"
						cy="0.7604166666666667"
						fill="black"
						r="0.2604166666666667"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.8125"
						rx="0.1625"
						ry="0.1625"
						width="0.625"
						x="0.5"
						y="0.3125"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.6770833333333334"
						rx="0.1625"
						ry="0.1625"
						width="0.5208333333333334"
						x="0.5"
						y="0.34375"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.625"
						rx="0.3125"
						ry="0.3125"
						width="1.1875"
						x="0.21875"
						y="0.5"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.5208333333333334"
						rx="0.0968967013888889"
						ry="0.0968967013888889"
						width="0.9895833333333334"
						x="0.265625"
						y="0.5"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.85" cy="0.85" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7916666666666667"
						cy="0.7916666666666667"
						fill="black"
						r="0.20833333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.65"
						rx="0.13"
						ry="0.13"
						width="0.5"
						x="0.6"
						y="0.45"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.5416666666666667"
						rx="0.13"
						ry="0.13"
						width="0.4166666666666667"
						x="0.5833333333333333"
						y="0.45833333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.95"
						x="0.375"
						y="0.6"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4166666666666667"
						rx="0.0920138888888889"
						ry="0.0920138888888889"
						width="0.7916666666666666"
						x="0.3958333333333333"
						y="0.5833333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle
						cx="0.8333333333333334"
						cy="0.8333333333333334"
						fill="black"
						r="0.2916666666666667"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7777777777777779"
						cy="0.7777777777777779"
						fill="black"
						r="0.24305555555555558"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.75"
						rx="0.15"
						ry="0.15"
						width="0.5833333333333334"
						x="0.5416666666666666"
						y="0.375"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.625"
						rx="0.15"
						ry="0.15"
						width="0.48611111111111116"
						x="0.5347222222222222"
						y="0.3958333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5833333333333334"
						rx="0.2916666666666667"
						ry="0.2916666666666667"
						width="1.0833333333333333"
						x="0.2916666666666667"
						y="0.5416666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.48611111111111116"
						rx="0.091772762345679"
						ry="0.091772762345679"
						width="0.9027777777777778"
						x="0.3263888888888889"
						y="0.5347222222222222"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.84375" cy="0.84375" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7864583333333334"
						cy="0.7864583333333334"
						fill="black"
						r="0.20833333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.65625"
						rx="0.13125"
						ry="0.13125"
						width="0.5"
						x="0.59375"
						y="0.4375"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.546875"
						rx="0.13125"
						ry="0.13125"
						width="0.4166666666666667"
						x="0.578125"
						y="0.4479166666666667"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.96875"
						x="0.359375"
						y="0.59375"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4166666666666667"
						rx="0.08875868055555558"
						ry="0.08875868055555558"
						width="0.8072916666666667"
						x="0.3828125"
						y="0.578125"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.85" cy="0.85" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7916666666666667"
						cy="0.7916666666666667"
						fill="black"
						r="0.20833333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.65"
						rx="0.13"
						ry="0.13"
						width="0.5"
						x="0.6"
						y="0.45"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.5416666666666667"
						rx="0.13"
						ry="0.13"
						width="0.4166666666666667"
						x="0.5833333333333333"
						y="0.45833333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.95"
						x="0.375"
						y="0.6"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4166666666666667"
						rx="0.0876736111111111"
						ry="0.0876736111111111"
						width="0.7916666666666666"
						x="0.3958333333333333"
						y="0.5833333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle
						cx="0.8636363636363636"
						cy="0.8636363636363636"
						fill="black"
						r="0.22727272727272727"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.803030303030303"
						cy="0.803030303030303"
						fill="black"
						r="0.1893939393939394"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5909090909090909"
						rx="0.11818181818181818"
						ry="0.11818181818181818"
						width="0.45454545454545453"
						x="0.6363636363636364"
						y="0.5"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4924242424242425"
						rx="0.11818181818181818"
						ry="0.11818181818181818"
						width="0.3787878787878788"
						x="0.6136363636363636"
						y="0.5"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.45454545454545453"
						rx="0.22727272727272727"
						ry="0.22727272727272727"
						width="0.8636363636363636"
						x="0.4318181818181818"
						y="0.6363636363636364"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-44"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.3787878787878788"
						rx="0.0869203397612488"
						ry="0.0869203397612488"
						width="0.7196969696969697"
						x="0.4431818181818182"
						y="0.6136363636363636"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.875" cy="0.875" fill="black" r="0.20833333333333334" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.8125"
						cy="0.8125"
						fill="black"
						r="0.17361111111111113"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5416666666666666"
						rx="0.10833333333333334"
						ry="0.10833333333333334"
						width="0.4166666666666667"
						x="0.6666666666666666"
						y="0.5416666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4513888888888889"
						rx="0.10833333333333334"
						ry="0.10833333333333334"
						width="0.34722222222222227"
						x="0.6388888888888888"
						y="0.5347222222222222"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.4166666666666667"
						rx="0.20833333333333334"
						ry="0.20833333333333334"
						width="0.7916666666666666"
						x="0.4791666666666667"
						y="0.6666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.34722222222222227"
						rx="0.08634741512345678"
						ry="0.08634741512345678"
						width="0.6597222222222222"
						x="0.4826388888888889"
						y="0.6388888888888888"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle
						cx="0.8392857142857143"
						cy="0.8392857142857143"
						fill="black"
						r="0.19642857142857142"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7827380952380953"
						cy="0.7827380952380953"
						fill="black"
						r="0.1636904761904762"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.5178571428571429"
						rx="0.10357142857142858"
						ry="0.10357142857142858"
						width="0.39285714285714285"
						x="0.6428571428571429"
						y="0.5178571428571429"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.4315476190476191"
						rx="0.10357142857142858"
						ry="0.10357142857142858"
						width="0.3273809523809524"
						x="0.6190476190476191"
						y="0.5148809523809524"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.39285714285714285"
						rx="0.19642857142857142"
						ry="0.19642857142857142"
						width="0.7678571428571429"
						x="0.45535714285714285"
						y="0.6428571428571429"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.3273809523809524"
						rx="0.08576920351473921"
						ry="0.08576920351473921"
						width="0.6398809523809524"
						x="0.46279761904761907"
						y="0.6190476190476191"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.85" cy="0.85" fill="black" r="0.175" />
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7916666666666667"
						cy="0.7916666666666667"
						fill="black"
						r="0.14583333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.45"
						rx="0.09"
						ry="0.09"
						width="0.35"
						x="0.675"
						y="0.575"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.375"
						rx="0.09"
						ry="0.09"
						width="0.2916666666666667"
						x="0.6458333333333334"
						y="0.5625"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.35"
						rx="0.175"
						ry="0.175"
						width="0.65"
						x="0.525"
						y="0.675"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.2916666666666667"
						rx="0.08485243055555552"
						ry="0.08485243055555552"
						width="0.5416666666666667"
						x="0.5208333333333334"
						y="0.6458333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-round-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle
						cx="0.8333333333333334"
						cy="0.8333333333333334"
						fill="black"
						r="0.16666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-round-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.7777777777777779"
						cy="0.7777777777777779"
						fill="black"
						r="0.1388888888888889"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-mobile-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.43333333333333335"
						rx="0.08666666666666667"
						ry="0.08666666666666667"
						width="0.3333333333333333"
						x="0.6666666666666666"
						y="0.5666666666666667"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-mobile-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.36111111111111116"
						rx="0.08666666666666667"
						ry="0.08666666666666667"
						width="0.2777777777777778"
						x="0.6388888888888888"
						y="0.5555555555555556"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-status-typing-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.3333333333333333"
						rx="0.16666666666666666"
						ry="0.16666666666666666"
						width="0.6333333333333333"
						x="0.5166666666666667"
						y="0.6666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-decoration-status-typing-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<rect
						fill="black"
						height="0.2777777777777778"
						rx="0.08429783950617281"
						ry="0.08429783950617281"
						width="0.5277777777777778"
						x="0.513888888888889"
						y="0.6388888888888888"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.5208333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.5208333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle cx="0.8125" cy="0.8125" fill="black" r="0.3125" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-16"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.5208333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<rect
						fill="black"
						height="0.625"
						rx="0.3125"
						ry="0.3125"
						width="1.1875"
						x="0.21875"
						y="0.5"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4833333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4833333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle cx="0.85" cy="0.85" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-20"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4833333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.95"
						x="0.375"
						y="0.6"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4583333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4583333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.8333333333333334"
						cy="0.8333333333333334"
						fill="black"
						r="0.2916666666666667"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-24"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4583333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<rect
						fill="black"
						height="0.5833333333333334"
						rx="0.2916666666666667"
						ry="0.2916666666666667"
						width="1.0833333333333333"
						x="0.2916666666666667"
						y="0.5416666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4270833333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4270833333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle cx="0.84375" cy="0.84375" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.4270833333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.96875"
						x="0.359375"
						y="0.59375"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4083333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4083333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle cx="0.85" cy="0.85" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-40"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.4083333333333334"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<rect
						fill="black"
						height="0.5"
						rx="0.25"
						ry="0.25"
						width="0.95"
						x="0.375"
						y="0.6"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.3958333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.3958333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle cx="0.875" cy="0.875" fill="black" r="0.20833333333333334" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-48"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.3958333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<rect
						fill="black"
						height="0.4166666666666667"
						rx="0.20833333333333334"
						ry="0.20833333333333334"
						width="0.7916666666666666"
						x="0.4791666666666667"
						y="0.6666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.3869047619047619"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.3869047619047619"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.8392857142857143"
						cy="0.8392857142857143"
						fill="black"
						r="0.19642857142857142"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-56"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.3869047619047619"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<rect
						fill="black"
						height="0.39285714285714285"
						rx="0.19642857142857142"
						ry="0.19642857142857142"
						width="0.7678571428571429"
						x="0.45535714285714285"
						y="0.6428571428571429"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.37083333333333335"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.37083333333333335"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle cx="0.85" cy="0.85" fill="black" r="0.175" />
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.33333333333333337"
						cy="0.33333333333333337"
						fill="white"
						r="0.33333333333333337"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="black"
						r="0.37083333333333335"
					/>
					<circle
						cx="0.6666666666666667"
						cy="0.6666666666666667"
						fill="white"
						r="0.33333333333333337"
					/>
					<rect
						fill="black"
						height="0.35"
						rx="0.175"
						ry="0.175"
						width="0.65"
						x="0.525"
						y="0.675"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.35833333333333334"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-status-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.35833333333333334"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.8333333333333334"
						cy="0.8333333333333334"
						fill="black"
						r="0.16666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-diagonal-facepile-typing-120"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle
						cx="0.3333333333333333"
						cy="0.3333333333333333"
						fill="white"
						r="0.3333333333333333"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="black"
						r="0.35833333333333334"
					/>
					<circle
						cx="0.6666666666666666"
						cy="0.6666666666666666"
						fill="white"
						r="0.3333333333333333"
					/>
					<rect
						fill="black"
						height="0.3333333333333333"
						rx="0.16666666666666666"
						ry="0.16666666666666666"
						width="0.6333333333333333"
						x="0.5166666666666667"
						y="0.6666666666666666"
					/>
				</mask>
				<mask
					id="svg-mask-status-online-mobile"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect
						fill="white"
						height="1"
						rx="0.1875"
						ry="0.125"
						width="1"
						x="0"
						y="0"
					/>
					<rect
						fill="black"
						height="0.5"
						width="0.75"
						x="0.125"
						y="0.16666666666666666"
					/>
					<ellipse
						cx="0.5"
						cy="0.8333333333333334"
						fill="black"
						rx="0.125"
						ry="0.08333333333333333"
					/>
				</mask>
				<mask
					id="svg-mask-status-online"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
				</mask>
				<mask
					id="svg-mask-status-idle"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.25" cy="0.25" fill="black" r="0.375" />
				</mask>
				<mask
					id="svg-mask-status-dnd"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<rect
						fill="black"
						height="0.25"
						rx="0.125"
						ry="0.125"
						width="0.75"
						x="0.125"
						y="0.375"
					/>
				</mask>
				<mask
					id="svg-mask-status-offline"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.5" cy="0.5" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-status-streaming"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<polygon fill="black" points="0.35,0.25 0.78301275,0.5 0.35,0.75" />
				</mask>
				<mask
					id="svg-mask-status-typing"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect
						cx="0"
						cy="0"
						fill="white"
						height="1"
						rx="0.2"
						ry="0.5"
						width="1"
					/>
				</mask>
				<mask
					id="svg-mask-status-screenshare"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<path
						d="M0.5 0.71875C0.5 0.649716 0.555966 0.59375 0.625 0.59375H1.0V1.0H0.5V0.71875Z"
						fill="black"
					/>
				</mask>
				<mask
					id="svg-mask-avatar-voice-call-80"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" r="0.5" />
					<circle cx="0.85" cy="0.85" fill="black" r="0.2" />
				</mask>
				<mask
					id="svg-mask-avatar-call-icon"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" opacity="1" r="0.5" />
					<circle cx="0.85" cy="0.15" fill="black" r="0.2" />
				</mask>
				<mask
					id="svg-mask-avatar-call-icon-32"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<circle cx="0.5" cy="0.5" fill="white" opacity="0.5" r="0.5" />
					<circle cx="0.8" cy="0.25" fill="black" r="0.325" />
				</mask>
				<mask
					id="svg-mask-sticker-rounded-rect"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<path
						d="M0 0.26087C0 0.137894 0 0.0764069 0.0382035 0.0382035C0.0764069 0 0.137894 0 0.26087 0H0.73913C0.862106 0 0.923593 0 0.961797 0.0382035C1 0.0764069 1 0.137894 1 0.26087V0.73913C1 0.862106 1 0.923593 0.961797 0.961797C0.923593 1 0.862106 1 0.73913 1H0.26087C0.137894 1 0.0764069 1 0.0382035 0.961797C0 0.923593 0 0.862106 0 0.73913V0.26087Z"
						fill="white"
					/>
				</mask>
				<mask
					id="svg-mask-chat-input-button-notification"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<circle cx="0.85" cy="0.85" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-sticker-shop-notification"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<circle cx="0.9" cy="0.9" fill="black" r="0.5" />
				</mask>
				<mask
					id="svg-mask-autocomplete-emoji-upsell-emoji"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<circle cx="1.33" cy="0.5" fill="black" r="0.5833333333333334" />
				</mask>
				<mask
					id="svg-mask-event-ticket"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<path
						d="M0 0.12C0 0.0779961 0 0.0569941 0.00408726 0.0409507C0.00768251 0.0268386 0.0134193 0.015365 0.0204754 0.00817451C0.028497 0 0.038998 0 0.06 0H0.94C0.961002 0 0.971503 0 0.979525 0.00817451C0.986581 0.015365 0.992318 0.0268386 0.995913 0.0409507C1 0.0569941 1 0.0779961 1 0.12V0.45C0.986193 0.45 0.975 0.472386 0.975 0.5C0.975 0.527614 0.986193 0.55 1 0.55V0.88C1 0.922004 1 0.943006 0.995913 0.959049C0.992318 0.973161 0.986581 0.984635 0.979525 0.991826C0.971503 1 0.961002 1 0.94 1H0.0600001C0.0389981 1 0.028497 1 0.0204754 0.991826C0.0134193 0.984635 0.00768251 0.973161 0.00408726 0.959049C0 0.943006 0 0.922004 0 0.88V0.55C0.0138071 0.55 0.025 0.527614 0.025 0.5C0.025 0.472386 0.0138071 0.45 0 0.45V0.12Z"
						fill="white"
					/>
				</mask>
				<mask
					id="svg-mask-guild-icon-with-channel-type"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<path
						d="M0 0.464C0 0.301585 0 0.220377 0.0316081 0.158343C0.0594114 0.103776 0.103776 0.0594114 0.158343 0.0316081C0.220377 0 0.301585 0 0.464 0H0.536C0.698415 0 0.779623 0 0.841657 0.0316081C0.896224 0.0594114 0.940589 0.103776 0.968392 0.158343C1 0.220377 1 0.301585 1 0.464V0.536C1 0.698415 1 0.779623 0.968392 0.841657C0.940589 0.896224 0.896224 0.940589 0.841657 0.968392C0.779623 1 0.698415 1 0.536 1H0.464C0.301585 1 0.220377 1 0.158343 0.968392C0.103776 0.940589 0.0594114 0.896224 0.0316081 0.841657C0 0.779623 0 0.698415 0 0.536V0.464Z"
						fill="white"
					/>
					<circle cx="0.9" cy="0.9" fill="black" r="0.5" />
				</mask>
				<mask
					id="svg-mask-content-inventory-card-face-pile-avatar"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" />
					<circle cx="1.33" cy="0.5" fill="black" r="0.65" />
				</mask>
				<mask
					id="svg-mask-guild-popout-activity-icon"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle cx="1.33" cy="0.5" fill="black" r="0.6" />
				</mask>
				<mask
					id="svg-mask-clan-icon"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" rx="0.3" ry="0.3" width="1" />
				</mask>
				<mask
					id="svg-mask-badged-guild-icon"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="1" width="1" x="0" y="0" />
					<circle cx="0.85" cy="0.15" fill="black" r="0.25" />
				</mask>
				<mask
					id="svg-mask-forward-attachment-pile-overflow"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<path
						d="M0.142857 0C0.0639593 0 0 0.0639593 0 0.142857V0.857143C0 0.936041 0.0639593 1 0.142857 1H0.554564C0.520632 0.962089 0.5 0.912025 0.5 0.857143V0.714286C0.5 0.595939 0.595939 0.5 0.714286 0.5H0.857143C0.912025 0.5 0.962089 0.520632 1 0.554564V0.142857C1 0.0639593 0.936041 0 0.857143 0H0.142857Z"
						fill="white"
					/>
				</mask>
				<mask
					id="svg-mask-content-image-60"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.8666666666666667"
						cy="0.8666666666666667"
						fill="black"
						r="0.23333333333333334"
					/>
				</mask>
				<mask
					id="svg-mask-content-image-72"
					maskContentUnits="objectBoundingBox"
					viewBox="0 0 1 1"
				>
					<rect fill="white" height="100%" width="100%" x="0" y="0" />
					<circle
						cx="0.8333333333333334"
						cy="0.8333333333333334"
						fill="black"
						r="0.2777777777777778"
					/>
				</mask>
			</svg>
		</div>
	);
}
